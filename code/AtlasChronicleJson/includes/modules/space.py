...
class SpaceObj(object):
   def __init__(self, key, name, width = 0, height = 0, oType = 'loi'):
...
class Restriction(object):
   def __init__(self, object1, object2, minLen, maxLen,
                typed = True, center1 = (0,0), center2 = (0,0)):
   ...
   def update(self):
      ...
      body0 = self.objects[0].body
      body1 = self.objects[1].body
      pos0 = body0.position
      pos1 = body1.position
      vel0 = body0.velocity
      vel1 = body1.velocity
      
      ipos0 = [pos0[0] + vel0[0], pos0[1] + vel0[1]]
      ipos1 = [pos1[0] + vel1[0], pos1[1] + vel1[1]]
      
      idist = distance(ipos0, ipos1)
      idX = abs(ipos0[0] - ipos1[0])
      idY = abs(ipos0[1] - ipos1[1])
      
      td = 0
      if idist < self.dimensions[0]:
         td = self.dimensions[0]
      elif idist > self.dimensions[1]:
         td = self.dimensions[1]
      ...
      if td == self.dimensions[0]:
         print self.objects[0].key, self.objects[1].key, td
   ...
   def draw(self, canvas, scale):
      # draw an arrow? maybe just a line for both bodies
      ...
      end1 = (p1[0] + v1[0], p1[1] + v1[1])
      end2 = (p2[0] + v2[0], p2[1] + v2[1])
      
      color = COLORS['spring']
      if self.typed:
         color = COLORS['springT']

      pygame.draw.line(canvas, color, p1, end1)
      pygame.draw.line(canvas, color, p2, end2)
...
class SpaceManager(object):
   def __init__(self, animate=True, pause=False):
      
      self.useSprings = False
   ...
   def draw(self):
      ...
      if self.useSprings:
         ...
         for r in self.restrictions:
            self.restrictions[r].draw(self.canvas, self.spaceScale)
   ...
   def simulate(self):
      ...
      if self.useSprings:
         ...
         for o in self.objects:
            #self.objects[o].body.reset_forces()
            tempVels[o] = [0,0]
            tempCount[o] = 0
         ...
         for r in self.restrictions:
            vel = self.restrictions[r].update()
            b1 = self.restrictions[r].objects[0].key
            b2 = self.restrictions[r].objects[1].key
            tempCount[b1] += 1
            tempCount[b2] += 1
            tempVels[b1] = [tempVels[b1][0] + vel[0], tempVels[b1][1] + vel[1]]
            tempVels[b2] = [tempVels[b2][0] - vel[0], tempVels[b2][1] - vel[1]]
            
         
         for o in self.objects:
            self.objects[o].body.apply_force(tempVels[o], (0,0))#[x / tempCount[o] for x in tempVels[o]], (0,0))
      ...
      for obj in self.objects:
         ...
         if body.velocity.x < self.minVelocity and \
            ...
            if body.velocity.x > self.maxVelocity:
               body.velocity.x = self.maxVelocity
            if body.velocity.y > self.maxVelocity:
               body.velocity.y = self.maxVelocity
   ...
   def handleEvent(self, e, offset=None):
      ...
      if e.type == KEYDOWN:
         ...
         if self.state == "identify" or self.state == "manual":
            ...
            for obj in self.objects:
               dist = distance(loc, self.objects[obj].body.position)
   ...
   def addObject(self, key, name, width = 0, height = 0):
      obj = SpaceObj(key, name, width, height)
   ...
   def addSSpring(self, rKey, rInfo):
      if self.useSprings:
         ss = SlideSpring(self.objects[rInfo["first"]], self.objects[rInfo["second"]],
                           rInfo["dimensions"]["min"],
                           rInfo["dimensions"]["max"], rInfo["typed"])
         s, trash = ss.get()
         # Force the initial addition, rest is handled in update.
         self.space.add(s)
         self.sSprings[rKey] = ss
      else:
         self.addRestriction(rKey, rInfo)
   
   def addRestriction(self, rKey, rInfo):
      r = Restriction(self.objects[rInfo["first"]], self.objects[rInfo["second"]],
                      rInfo["dimensions"]["min"],
                      rInfo["dimensions"]["max"], rInfo["typed"])
      self.restrictions[rKey] = r
